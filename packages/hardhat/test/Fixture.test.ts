...
*/
  ...
  describe("Swapping", function () {
    ...
    it("reverts when user has not approved enough input tokens for swap", async () => {
      ...
      await expect(
        swap.swapExactTokensForTokens(ethers.parseEther("5"), 0n, [token1.target, token2.target], deployer, deadline),
    ...
    it("emits SwapSuccess event with correct values after a successful swap", async () => {
      ...
      const event = receipt?.logs
        .map((log: any) => swap.interface.parseLog(log))
        .find((e: any) => e && e.name === "SwapSuccess") as undefined | { args: any };

      if (!event) throw new Error("SwapSuccess event not found");
